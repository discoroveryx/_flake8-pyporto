...
class TestChecks(unittest.TestCase):
    ...
    def _results(self, filename):
        testContent = open(BASE_DIR + '\\tests\\' + filename).read()
        tree = ast.parse(testContent)
        plugin = PyPorto(tree)
        return {f"{line}:{col} {msg}" for line, col, msg, _ in plugin.run()}

    @mock.patch.object(PyPorto, 'get_filename_wrapper')
    def test_unary_not_equality(self, mocked_filename):
        mocked_filename.return_value = 'example_case_1_task.py'

        ret = self._results('example_case_1_task.py')
        self.assertEqual(ret, {'1:1 PYP001 is bad'})
